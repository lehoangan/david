...
class Parser(report_sxw.rml_parse):
    ...
    def get_journal(self, form):
        ...
        select_str = """
                SELECT
                        distinct (jou.id) as id,
                        jou.name,
                        SUM(vou.amount) as total
                FROM account_voucher vou
                    join account_journal jou on (vou.journal_id=jou.id)
                %s
                GROUP BY jou.name,jou.id
                order by jou.name
        """%where_str
    ...
    def get_user_amount_due(self, form, journal):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE vou.state = 'draft' AND vou.type = 'receipt' '''
        else:
        ...
        if journal['id']:
            where_str = '%s %s'%(where_str, ' AND vou.journal_id = %s'%journal['id'])
    ...
    def get_market(self, form, journal):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE vou.state = 'draft' AND vou.type = 'receipt' '''
        else:
    ...
    def get_market_amount_due(self, form, market, journal):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE vou.state = 'draft' AND vou.type = 'receipt' '''
        else:
    ...
    def get_customer(self, form, market, journal):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE vou.state = 'draft' AND vou.type = 'receipt'  '''
        else:
    ...
    def get_detail(self, form, market, partner, journal):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE vou.state = 'draft' AND vou.type = 'receipt'  '''
        else:

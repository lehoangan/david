...
class Parser(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'total_supplier': self.total_supplier,
    ...
    def get_market(self, form):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE inv.state = 'draft' AND inv.type = 'in_invoice' '''
        else:
            if form['inv_state'] != 'all':
                where_str = ''' WHERE inv.state = '%s' AND inv.type = 'in_invoice' '''%form['inv_state']
            else:
                where_str = ''' WHERE inv.state not in ('draft', 'cancel') AND inv.type = 'in_invoice' '''
        if form['frial']:
    ...
    def total_customer(self, form, partner_id):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE inv.state = 'draft' AND inv.type = 'in_invoice' AND inv.partner_id=%s '''%partner_id
        else:
            if form['inv_state'] != 'all':
                where_str = ''' WHERE inv.state = '%s' AND inv.type = 'in_invoice' AND inv.partner_id=%s '''%(form['inv_state'], partner_id)
            else:
                where_str = ''' WHERE inv.state not in ('draft', 'cancel') AND inv.type = 'in_invoice' AND inv.partner_id=%s '''%partner_id
    ...
    def get_invoice(self, form, state_id, partner):
        ...
        select_str = """
                 SELECT
                        ...
                        inv.number,
                        inv.date_invoice,
                        inv.supplier_invoice_number as ref
                FROM (
                    account_invoice inv
                  join res_partner part on (inv.partner_id=part.id)
                  left join res_partner_res_partner_category_rel rel on (rel.partner_id=part.id)
    ...
    def total_supplier(self, form, supplier_id):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE inv.state = 'draft' AND inv.type = 'in_invoice'  '''
        else:
            if form['inv_state'] != 'all':
                ...
                where_str = ''' WHERE inv.state not in ('draft', 'cancel') AND inv.type = 'in_invoice' '''
        where_str = '%s AND inv.partner_id = %s '%(where_str, supplier_id)
        if form['frial']:
            where_str = '%s %s'%(where_str, ' AND part.frial = %s'%form['frial'])
        if form['partner_id']:
            where_str = '%s %s'%(where_str, ' AND inv.partner_id = %s'%form['partner_id'][0])
    ...
    def total_product(self, form, supplier_id):
        where_str = ''
        if form['state'] == 'draft':
            where_str = ''' WHERE inv.state = 'draft' AND inv.type = 'in_invoice'  '''
        else:
        ...
        sql = '''
            SELECT
                prod.id,
                prodt.name,
                sum(ivl.quantity) as quantity,
                sum(ivl.product_uos_qty) as unit,
                sum(ivl.discount_kg) as disc_kg,
                sum(ivl.price_unit * (ivl.discount/100 * (ivl.quantity -ivl. discount_kg) + ivl.discount_kg)) as disc_amount
            FROM
                account_invoice_line ivl
                JOIN product_product prod on (prod.id=ivl.product_id)
                JOIN product_template prodt on (prod.product_tmpl_id=prodt.id)
            WHERE
                invoice_id in (%s)
            GROUP BY prod.id,
                    prodt.name
